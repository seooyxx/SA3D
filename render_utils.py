...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses, desc='Render {}...'.format(seg_type))):
        ...
        # 여기거나 아래 기존에 뺐던 데에서 빼는게 맞는거 같은데 이 값을 어떻게 point cloud 나  mesh로 표현하지? ;;
    ...
    # 여기가 맞는거 같기도 함
    # things to add - RGB 값에 BG맵 값 곱해서 배경 날리고, RGB 값, depth 값 조합해서 rgbd 이미지 만들기
    ...
    # numpy 배열 정리
    # expanded_bgmaps = np.repeat(bgmaps, 3, axis=3)
    # rgbs_without_bg = np.multiply(rgbs, expanded_bgmaps)
    # rgbds = np.concatenate((rgbs_without_bg, depths), axis=3)

    # # 저장
    # output_dir = 'output_data'
    # os.makedirs(output_dir, exist_ok=True)

    # rgbds_2d = rgbds.reshape(-1, 4)

    # num_files = len(rgbds)
    # batch_size = len(rgbds_2d) // num_files
    ...
    #     # 파일명
    #     filename = os.path.join(output_dir, f'array_{i}.txt')

    #     # 해당 범위의 데이터를 파일로 저장
    #     np.savetxt(filename, rgbds_2d[start:end])
...
def render_fn(args, cfg, ckpt_name, flag, e_flag, num_obj, data_dict, render_viewpoints_kwargs, seg_type='seg_density'):
    ...
    # after segment일 경우에는 color는 여기에서 seg_on_rgb를 빼야할거같다.
    if seg_type == 'seg_img':
        seg_on_rgb = []
        if args.dump_images:
        ...
        # rbgs, depths, bgmaps, segs, seg_on_rgb 저장
        output_dir = 'seg_output_data'
        os.makedirs(output_dir, exist_ok=True)
        
        rgbs_filename = os.path.join(output_dir, f'rbgs.txt')
        depths_filename = os.path.join(output_dir, f'depths.txt') # 이 depths랑
        bgmaps_filename = os.path.join(output_dir, f'bgmaps.txt')
        segs_filename = os.path.join(output_dir, f'segs.txt')
        seg_on_rgb_filename = os.path.join(output_dir, f'seg_on_rgb.txt') # ui에서 marks선택한 후에 -> reb값 이거 빼면될듯
